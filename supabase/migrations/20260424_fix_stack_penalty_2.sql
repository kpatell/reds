...
BEGIN
    ...
    IF v_non_null_count >= 6 AND v_owner_id != p_player_id::text THEN
        RETURN jsonb_build_object('success', false, 'action', jsonb_build_object('description', 'Hand is full (Max 6 cards). Cannot stack opponent cards!'));
    END IF;
