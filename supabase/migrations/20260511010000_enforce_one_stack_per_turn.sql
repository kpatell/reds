-- Prevent consecutive stacks: if last_game_action was a stack_success, the
-- next stack attempt is treated as a wrong stack (penalty card issued to caller).
-- A normal turn action (swap/discard) overwrites last_game_action and unlocks stacking.
...
BEGIN
  ...
  -- A stack can only happen once per turn. If the previous action was a
  -- stack_success, the top card was placed via a stack and further stacking is
  -- locked until a normal swap/discard overwrites last_game_action.
