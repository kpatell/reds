CREATE OR REPLACE FUNCTION resolve_stack_transfer(
  p_game_id uuid,
  p_player_id uuid,
  p_hand_card_id text
) RETURNS jsonb
...
BEGIN
    ...
    v_transfer := v_game.pending_stack_transfer;
    IF v_transfer IS NULL OR jsonb_typeof(v_transfer) = 'null' OR (v_transfer->>'playerId') != p_player_id::text THEN
        RETURN jsonb_build_object('success', false, 'error', 'No pending transfer for this player');
    END IF;

    v_target_player_id := v_transfer->>'targetPlayerId';
    v_target_slot_index := (v_transfer->>'slotIndex')::INT;
    v_players := v_game.players;
    v_player_hand := v_players -> p_player_id::text -> 'hand';
    v_target_hand := v_players -> v_target_player_id -> 'hand';
    ...
    IF v_card_index = -1 THEN
        RETURN jsonb_build_object('success', false, 'error', 'Card not found in your hand');
    END IF;

    -- Replace card with null in player's hand
    v_player_hand := jsonb_set(v_player_hand, ARRAY[v_card_index::text], 'null'::jsonb);
    
    -- Trim trailing nulls
    WHILE jsonb_array_length(v_player_hand) > 4 AND jsonb_typeof(v_player_hand -> (jsonb_array_length(v_player_hand) - 1)) = 'null' LOOP
        v_player_hand := v_player_hand - (jsonb_array_length(v_player_hand) - 1);
    END LOOP;
    
    v_players := jsonb_set(v_players, ARRAY[p_player_id::text, 'hand'], v_player_hand);

    -- Put card into target's hand at slotIndex
    v_target_hand := jsonb_set(v_target_hand, ARRAY[v_target_slot_index::text], v_card);
    
    -- Trim trailing nulls
    WHILE jsonb_array_length(v_target_hand) > 4 AND jsonb_typeof(v_target_hand -> (jsonb_array_length(v_target_hand) - 1)) = 'null' LOOP
        v_target_hand := v_target_hand - (jsonb_array_length(v_target_hand) - 1);
    END LOOP;
    
    v_players := jsonb_set(v_players, ARRAY[v_target_player_id, 'hand'], v_target_hand);

    -- Update database
    UPDATE games
    SET players = v_players,
        pending_stack_transfer = NULL,
        deck = v_game.deck, -- FIX: Prevent TOAST omission
        last_action_at = now(),
        last_game_action = jsonb_build_object(
            'playerId', p_player_id,
            'actionType', 'stack_transfer',
            'description', 'Gave card to opponent!',
...
BEGIN
    ...
    IF NOT FOUND THEN RETURN jsonb_build_object('success', false, 'action', jsonb_build_object('description', 'Game not found')); END IF;
    ...
    IF jsonb_array_length(v_discard_pile) = 0 THEN RETURN jsonb_build_object('success', false, 'action', jsonb_build_object('description', 'Discard pile is empty!')); END IF;
    ...
    LOOP
        ...
        LOOP
            ...
            END IF;
        END LOOP;
        IF v_owner_id IS NOT NULL THEN EXIT; END IF;
    END LOOP;

    IF v_owner_id IS NULL THEN RETURN jsonb_build_object('success', false, 'action', jsonb_build_object('description', 'Card not found in any hand.')); END IF;
