-- Restore pending_stack_transfer for cross-hand stacks.
-- When a player stacks a card from the opponent's hand, set pending_stack_transfer
-- so the stacker must give one of their own cards back to the opponent.
-- Penalty path explicitly clears pending_stack_transfer = NULL for safety.
  ...
  IF v_new_action IS NULL THEN
    ...
    ELSE
      ...
      SET players                = v_players,
          ...
          -- Cross-hand stack: stacker must pass a card to the owner
