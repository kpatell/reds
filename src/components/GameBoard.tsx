...
export function GameBoard({ gameState, onDraw, onDiscard, onSwap, onReady }: GameBoardProps) {
    ...
    return (
        <div className="flex flex-col h-screen w-full max-w-6xl mx-auto p-2 gap-2 overflow-hidden relative">
            ...
            <div className="flex-1 flex items-start justify-center rotate-180 min-h-0">
                {opponent ? (
                    <PlayerHand
                        ...
                        cardClassName="w-16 h-24 sm:w-20 sm:h-32 md:w-24 md:h-36"
                    />
                ) : (
            ...
            <div className="flex-none flex items-center justify-center gap-8 sm:gap-16 py-2 relative w-full z-10">
                    ...
                    <div className="absolute left-2 lg:left-12 top-1/2 -translate-y-1/2 flex flex-col items-center gap-2 animate-in slide-in-from-left-10 fade-in duration-300 z-20">
                        ...
                        <div className="flex flex-col items-center gap-2 bg-[var(--color-surface)]/90 p-2 rounded-xl backdrop-blur-sm border border-[var(--color-border)] shadow-xl">
                            <Card
                                ...
                                className="w-16 h-24 sm:w-20 sm:h-32 md:w-24 md:h-36"
                            />
                ...
                >
                    <div className="w-16 h-24 sm:w-20 sm:h-32 md:w-24 md:h-36 bg-[var(--color-primary)] rounded-xl border-2 border-white/10 shadow-lg flex items-center justify-center">
                        <span className="text-white font-bold text-lg sm:text-xl">{gameState.deck.length}</span>
                    </div>
                    {/* Stack effect */}
                    <div className="absolute top-1 left-1 w-16 h-24 sm:w-20 sm:h-32 md:w-24 md:h-36 bg-[var(--color-primary)] rounded-xl -z-10 border-2 border-white/10"></div>
                    <div className="absolute top-2 left-2 w-16 h-24 sm:w-20 sm:h-32 md:w-24 md:h-36 bg-[var(--color-primary)] rounded-xl -z-20 border-2 border-white/10"></div>
                </div>
                ...
                >
                    {topCard ? (
                        ...
                        <div className="w-16 h-24 sm:w-20 sm:h-32 md:w-24 md:h-36 border-2 border-dashed border-[var(--color-border)] rounded-xl flex items-center justify-center text-[var(--color-text-muted)] text-xs sm:text-sm">
                            Discard
                        </div>
                    )}
                </div>
